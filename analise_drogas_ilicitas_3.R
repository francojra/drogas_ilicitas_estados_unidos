...
g1 <- ggplot(d, aes(x = Year, y = opioides, group = Code)) +
  geom_line(size = 0.8, col = "#e08214") +
  geom_point(size = 2.5, col = "#e08214") +
  labs(x = "Anos",
...
tg <- textGrob('Estados Unidos', gp = gpar(fontsize = 13, fontface = 'bold'))

plot_eua <- grid.arrange(g1, g2, g3, g4, ncol = 2,  # ncol se refere a número de colunas
                    top = tg)

# Salvar gráfico ---------------------------------------------------------------------------------------------------------------------------

ggsave("plot_eua.pdf", plot_eua, width = 15, height = 15)
